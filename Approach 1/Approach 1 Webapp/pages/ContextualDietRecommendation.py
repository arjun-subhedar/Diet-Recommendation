...
breakfast = pd.read_csv('/Users/anshavikhanna/Desktop/SEM 6/RecSys/Endterm Project/Approach 1/Approach 1 Webapp/dataset/breakfast.csv')
lunch = pd.read_csv('/Users/anshavikhanna/Desktop/SEM 6/RecSys/Endterm Project/Approach 1/Approach 1 Webapp/dataset/lunch.csv')
dinner = pd.read_csv('/Users/anshavikhanna/Desktop/SEM 6/RecSys/Endterm Project/Approach 1/Approach 1 Webapp/dataset/dinner.csv')
...
class Display:
    ...
    def display_calories(self,person):
        ...
        st.header(f'Optimal Calorie Intake (per day): {round(maintain_calories)} kcal') 
    ...
    def display_meal_plan(self,person,recommendations):
        ...
        for day_index in range(num_days):
            ...
            for meal_index in range(num_meals):
                ...
                with st.expander(f"{self.meal_index_to_name(meal_index)} :  {recommendations[meal_index].iloc[day_index]['Name']}"):
                    rounded_calories = round(recommendations[meal_index].iloc[day_index]['calories'], 2)
                    rounded_protein = round(recommendations[meal_index].iloc[day_index]['protein'], 2)
                    rounded_carbohydrates = round(recommendations[meal_index].iloc[day_index]['carbohydrates'], 2)
                    rounded_fat = round(recommendations[meal_index].iloc[day_index]['fat'], 2)
                    rounded_cholesterol = round(recommendations[meal_index].iloc[day_index]['cholesterol'], 2)
                    rounded_fiber = round(recommendations[meal_index].iloc[day_index]['fiber'], 2)
                    
                    st.write("Name:", recommendations[meal_index].iloc[day_index]['Name'])
                    st.write("Calories:", f"<span style='color: #004e9c'>{rounded_calories}</span>", "kcal", unsafe_allow_html=True)
                    st.write("Protein:", f"<span style='color: #004e9c'>{rounded_protein}</span>", "g", unsafe_allow_html=True)
                    st.write("Carbohydrates:", f"<span style='color: #004e9c'>{rounded_carbohydrates}</span>", "g", unsafe_allow_html=True)
                    st.write("Fat:", f"<span style='color: #004e9c'>{rounded_fat}</span>", "g", unsafe_allow_html=True)
                    st.write("Cholesterol:", f"<span style='color: #004e9c'>{rounded_cholesterol}</span>", "mg", unsafe_allow_html=True)
                    st.write("Fiber:", f"<span style='color: #004e9c'>{rounded_fiber}</span>", "mg", unsafe_allow_html=True)
                    st.write("URL:", recommendations[meal_index].iloc[day_index]['URL'])
